...
import {
  Flame, Check, AlertCircle,
  Zap, CheckCircle2, Snowflake,
  UtensilsCrossed,
  Bike, Car,
  Users,
} from 'lucide-react';
...
function ElapsedTimer({ createdAt, compact = false }: { createdAt: string; compact?: boolean }) {
  const { t } = useI18n();
  const [seconds, setSeconds] = React.useState(getElapsedSeconds(createdAt));
  React.useEffect(() => {
...
// ============ ORDER TICKET CARD ============
export function OrderTicketCard({ order, stationSlug, stationColor, onBump, onFire, onBumpAll, orderIndex }: {
