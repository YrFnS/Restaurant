...
function AnimatedStatCard({ icon, value, suffix, label, gradient, delay }: {
  icon: React.ReactNode; value: number; suffix: string; label: string; gradient: string; delay: number;
}) {
    ...
    >
      <Card className="border-border/50 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden rounded-xl">
        <CardContent className="p-4 sm:p-5">
          <div className={`size-10 rounded-lg bg-gradient-to-br ${gradient} flex items-center justify-center text-white mb-3 shadow-sm`}>
            {icon}
          </div>
...
export function HomeSection() {
  ...
  const settings = useRestaurantStore((s) => s.settings) as RestaurantSettings | null;
  ...
  const scrollSpecials = (direction: "start" | "end") => {
    ...
    specialsScrollRef.current.scrollBy({
      left: direction === "end" ? (isRTL ? -scrollAmount : scrollAmount) : (isRTL ? scrollAmount : -scrollAmount),
      behavior: "smooth",
    });
  };
    ...
    <div className="pb-4 space-y-6">
      ...
      <section className="relative overflow-hidden" ref={heroRef}>
        <div className="relative px-5 pt-8 pb-10 text-white">
          ...
          >
            ...
            <motion.div variants={heroChild} className="flex justify-center mb-4">
              <Badge
        ...
        >
          {loading
            ...
            : categories.map((cat, idx) => (
                ...
                >
                  <div className={`size-16 sm:size-20 rounded-2xl bg-gradient-to-br ${categoryGradients[cat.icon] || defaultGradient} flex items-center justify-center shadow-sm group-hover:shadow-md transition-shadow`}>
                    <span className="text-2xl sm:text-3xl drop-shadow-md">{cat.icon}</span>
                  </div>
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 px-4">
          {loading
            ? Array.from({ length: 6 }).map((_, i) => (
                <Card key={i} className="overflow-hidden border border-border/50 shadow-sm rounded-xl">
            ...
            : popularItems.map((item, itemIdx) => {
                ...
                return (
                  <motion.div
                    ...
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: itemIdx * 0.06, duration: 0.4, ease: "easeOut" }}
                      ...
                      <div className="relative h-36 overflow-hidden">
                        {item.image ? (
                          ...
                          <div className={`w-full h-full bg-gradient-to-br ${gradient} flex items-center justify-center`}>
                            <span className="text-4xl drop-shadow-md">{cat?.icon || "🍽️"}</span>
                          </div>
                        )}
                        ...
                        >
                          <Heart
                            className={`size-3.5 transition-all ${
                              isFav ? "text-red-400 fill-red-400 scale-110" : "text-white/80"
                            }`}
                          />
                        </button>
                        {/* Calorie badge */}
      ...
      <section className="px-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            { icon: <ShoppingCart className="size-5" />, value: settings?.statsOrdersServed || 0, suffix: '+', label: t.home.statsOrders, gradient: 'from-amber-500 to-orange-600' },
            { icon: <UtensilsCrossed className="size-5" />, value: categories.reduce((sum, c) => sum + c.items.length, 0), suffix: '+', label: t.home.statsMenuItems, gradient: 'from-orange-500 to-rose-600' },
            { icon: <Heart className="size-5" />, value: settings?.statsHappyCustomers || 0, suffix: '+', label: t.home.statsHappyCustomers, gradient: 'from-rose-500 to-pink-600' },
            { icon: <Clock className="size-5" />, value: settings?.statsYearsService || 0, suffix: '+', label: t.home.statsYears, gradient: 'from-teal-500 to-emerald-600' },
          ].map((stat, idx) => (
            <AnimatedStatCard key={idx} icon={stat.icon} value={stat.value} suffix={stat.suffix} label={stat.label} gradient={stat.gradient} delay={idx * 0.1} />
          ))}
        </div>
      </section>
        ...
        >
          ...
          <div className="relative z-10 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-bold text-lg sm:text-xl">{t.home.chefsRecommendation}</h3>
              <p className="text-white/80 text-sm mt-0.5 line-clamp-2">{t.home.chefsRecommendationDesc}</p>
            </div>
            <Button
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge className={`text-[9px] px-1.5 py-0 h-5 ${getRecentOrderStatusColor(order.status)}`}>
                              {t.orders.status[order.status as keyof typeof t.orders.status] || order.status}
                            </Badge>
                          </div>
                          <p className="text-xs text-muted-foreground truncate">
                            {order.items.length} {t.orders.items} · {currency}{order.total.toFixed(2)} · {t.home.orderPlacedAgo.replace("{{time}}", timeAgo(order.createdAt))}
                          </p>
                        </div>
                      </div>
                      <ChevronDown className={`size-4 text-muted-foreground -rotate-90 ${isRTL ? "rotate-90" : ""}`} />
                    </div>
                  </CardContent>
                </Card>
              </motion.div>
          ...
          {[
            ...
            >
              <div className="relative">
                <div className={`size-10 sm:size-12 rounded-full bg-gradient-to-br ${step.gradient} flex items-center justify-center shadow-sm`}>
      ...
      {settings && (
        <section className="px-4">
          <Card className="border border-border/50 shadow-sm overflow-hidden">
            <div className={`h-1.5 ${isOpen() ? "bg-gradient-to-r from-emerald-400 to-green-500" : "bg-gradient-to-r from-gray-400 to-gray-500"}`} />
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className={`size-10 rounded-full flex items-center justify-center ${isOpen() ? "bg-emerald-100 dark:bg-emerald-900/30" : "bg-gray-100 dark:bg-gray-900/30"}`}>
                    <Clock className={`size-5 ${isOpen() ? "text-emerald-600 dark:text-emerald-400" : "text-gray-500 dark:text-gray-400"}`} />
                  </div>
                  <div>
                    <h3 className="font-semibold text-sm">{t.home.restaurantHours}</h3>
      ...
      {testimonials.length > 0 && (
        <section className="space-y-3">
          ...
          <div className="flex gap-3 overflow-x-auto px-4 scroll-smooth snap-x snap-mandatory pb-2" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
                ...
                <Card className="border border-border/50 shadow-sm h-full rounded-xl hover:shadow-lg transition-shadow relative overflow-hidden">
                  ...
                  <CardContent className="p-5 relative z-10">
                    ...
                    <div className="flex items-center gap-0.5 mb-3">
                      ...
                      <span className="text-xs font-semibold text-muted-foreground ms-1.5">{review.stars}.0</span>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold">{locale === "ar" ? review.nameAr : review.nameEn}</p>
