...
import { TrackTab, HistoryTab } from "./orders/OrderList";
...
export function OrdersSection() {
  ...
  const handleTrack = () => fetchOrder(trackInput);
  const handleLookupHistory = () => fetchHistory();
  ...
  /* ─── Reorder for tracked completed order ─── */
  ...
  const handleReorder = () => {
    ...
    if (addedCount > 0) notifications.cartAdded(t.orders.reorderAdded);
  };
    ...
    <div className="min-h-screen pb-4" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <div className="sticky top-14 md:top-0 z-10 bg-background/95 backdrop-blur-md border-b border-border/50 px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" className="gap-1.5 text-muted-foreground" onClick={handleRefresh}>
            <RefreshCw className="size-3.5" />
            <span className="text-xs">{t.orders.refresh}</span>
          </Button>
        </div>
        <p className="text-xs text-muted-foreground mt-0.5">{t.orders.subtitle}</p>
      </div>
        ...
        <AnimatePresence mode="wait">
          {activeTab === "track" ? (
            ...
            <HistoryTab
              ...
              handleLookupHistory={handleLookupHistory}
