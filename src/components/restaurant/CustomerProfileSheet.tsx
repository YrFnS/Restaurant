...
/* ─── Component ─── */
export function CustomerProfileSheet({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const handleLookup = useCallback(async (phoneNum?: string) => {
              ...
              <Card className="border-border/50 shadow-sm">
                <CardContent className="pt-6 space-y-4">
                  <div className="text-center">
                    ...
                    <p className="font-medium text-sm">{t.profile.lookupPhone}</p>
                    <p className="text-xs text-muted-foreground mt-1">{t.profile.lookupDesc}</p>
                  </div>
                  ...
                  {lookupError && hasSearched && (
                    <div className="text-center p-3 rounded-lg bg-muted/50">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">{t.profile.notFoundDesc}</p>
                    </div>
                  )}
                </CardContent>
              </Card>
            ...
            <motion.div variants={fadeIn} initial="initial" animate="animate" className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Card className="border-border/50 shadow-sm">
                  <CardContent className="p-3 text-center">
                    ...
                    <p className="text-xl font-bold text-primary">{customer.loyaltyPoints}</p>
                    <p className="text-[10px] text-muted-foreground font-medium">{t.profile.loyaltyPoints}</p>
                  </CardContent>
                </Card>
                <Card className="border-border/50 shadow-sm">
                  <CardContent className="p-3 text-center">
                    ...
                    <p className="text-xl font-bold text-rose-600 dark:text-rose-400">{favorites.length}</p>
                    <p className="text-[10px] text-muted-foreground font-medium">{t.profile.favoritesCount}</p>
                  </CardContent>
                </Card>
                <Card className="border-border/50 shadow-sm">
                  <CardContent className="p-3 text-center">
                    ...
                    <p className="text-xl font-bold text-emerald-600 dark:text-emerald-400">{totalOrders}</p>
                    <p className="text-[10px] text-muted-foreground font-medium">{t.profile.totalOrders}</p>
                  </CardContent>
                </Card>
                <Card className="border-border/50 shadow-sm">
                  <CardContent className="p-3 text-center">
                    ...
                    <p className="text-xl font-bold text-teal-600 dark:text-teal-400">{currency}{customer.totalSpent.toFixed(0)}</p>
                    <p className="text-[10px] text-muted-foreground font-medium">{t.profile.totalSpent}</p>
                  </CardContent>
                </Card>
              </div>
              ...
              <Card className="border-border/50 shadow-sm">
                <CardContent className="p-4 space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-sm font-semibold">{t.profile.loyaltyProgress}</h4>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-muted-foreground">{t.profile.nextReward}</span>
                      <span className="font-medium">{Math.max(0, 100 - customer.loyaltyPoints)} {t.profile.pointsToNext}</span>
                    </div>
                    <Progress value={Math.min((customer.loyaltyPoints / 100) * 100, 100)} className="h-2" />
                    <p className="text-[10px] text-muted-foreground">
                      🥗 {t.profile.freeAppetizer} — 100 {t.profile.pointsNeeded}
                    </p>
                  </div>
                </CardContent>
              </Card>
                  ...
                  <CardContent className="p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="text-sm font-semibold">{t.profile.recentOrders}</h4>
                      </div>
                      ...
                      {orders.slice(0, 5).map((order) => (
                        <div key={order.id} className="flex items-center justify-between p-2 rounded-lg bg-muted/50 text-xs">
                          <div className="flex items-center gap-2 min-w-0">
                            <span className="font-semibold shrink-0">#{order.orderNumber}</span>
                            <Badge
              ...
              <div className="text-center">
                <p className="text-xs text-muted-foreground">
                  {t.profile.memberSince} {new Date(customer.createdAt).toLocaleDateString(locale === "ar" ? "ar-SA" : "en-US", { month: "long", year: "numeric" })}
                </p>
              </div>
            </motion.div>
          )}
