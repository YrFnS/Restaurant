...
export function RewardsSection() {
  ...
  const AMOUNT_PRESETS = settings?.giftCardAmounts
    ? settings.giftCardAmounts.split(",").map(Number).filter((n) => !isNaN(n) && n > 0)
    : [];
  ...
  const handleLookup = async (phoneNum?: string) => {
    ...
    setHasSearched(true);
    try {
      const res = await fetch(`/api/customers?phone=${encodeURIComponent(num.trim())}`);
      if (res.status === 404) {
        setCustomer(null);
        setLookupError(true);
        return;
  ...
  const handleSignUp = async () => {
    ...
    try {
      const res = await fetch("/api/customers", {
        ...
        body: JSON.stringify({ name: signUpName, phone: signUpPhone, email: signUpEmail || undefined }),
      });
      if (!res.ok) {
        const data = await res.json();
  ...
  const handleRedeem = async (points: number) => {
    ...
    try {
      const res = await fetch(`/api/customers/${customer.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ loyaltyPoints: customer.loyaltyPoints - points }),
      });
  ...
  const fetchGiftCards = async (purchaserName: string) => {
    ...
    setIsLoadingGc(true);
    try {
      const res = await fetch(`/api/gift-cards?purchaserName=${encodeURIComponent(purchaserName.trim())}`);
      if (!res.ok) throw new Error("Failed to fetch");
      const data = await res.json();
  ...
  const getNextReward = () => {
    ...
    for (const tier of rewardTiers) {
      if (points < tier.points) {
        return { ...tier, remaining: tier.points - points, progress: (points / tier.points) * 100 };
      }
    }
    return null;
  };
  ...
  const effectiveAmount = gcCustomAmount ? parseFloat(gcCustomAmount) || 0 : gcAmount;
        ...
        <motion.div variants={fadeIn}>
          <Tabs defaultValue="rewards" className="w-full">
            ...
            <TabsContent value="rewards" className="space-y-6 mt-4">
              ...
              {!customer ? (
                    ...
                    {!showSignUp ? (
                      <>
                        <div className="text-center">
                          ...
                          <p className="text-sm text-muted-foreground">{t.rewards.lookupDesc}</p>
                        </div>
                        ...
                        {lookupError && hasSearched && (
                          <div className="text-center">
                            <p className="text-sm text-muted-foreground mb-3">{t.rewards.notOnList}</p>
                      ...
                      <div className="space-y-4">
                        <div className="text-center">
                          ...
                          <p className="text-sm text-muted-foreground">{t.rewards.signUpDesc}</p>
                        </div>
                        ...
                        <Button className="w-full" onClick={handleSignUp} disabled={isSigningUp}>
                          {isSigningUp ? <Loader2 className="size-4 animate-spin me-2" /> : <UserPlus className="size-4 me-2" />}
                          {t.rewards.createAccount}
                        </Button>
                        <Button variant="ghost" className="w-full" onClick={() => setShowSignUp(false)}>
                          {t.common.back}
                        </Button>
                      </div>
                    )}
                ...
                <>
                  ...
                  <Card className="border-primary/20 shadow-sm">
                    <CardContent className="pt-6 space-y-4">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm text-muted-foreground">{t.rewards.welcomeBack}</p>
                          <p className="text-lg font-bold">{customer.name}</p>
                        </div>
                        <div className="text-end">
                          <p className="text-sm text-muted-foreground">{t.rewards.memberSince}</p>
                          <p className="text-sm font-medium">{format(new Date(customer.createdAt), "MMM yyyy")}</p>
                        </div>
                      </div>
                      ...
                      <div className="text-center py-2">
                        ...
                        <p className="text-4xl font-bold text-primary">{customer.loyaltyPoints}</p>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="text-center p-3 rounded-lg bg-muted/50">
                          <p className="text-lg font-bold">{currency}{customer.totalSpent.toFixed(0)}</p>
                          <p className="text-xs text-muted-foreground">{t.rewards.totalSpent}</p>
                        </div>
                        <div className="text-center p-3 rounded-lg bg-muted/50">
                          <p className="text-lg font-bold">{customer.visits}</p>
                          <p className="text-xs text-muted-foreground">{t.rewards.visits}</p>
                        </div>
                      </div>
                      ...
                      {nextReward && (
                        <div className="space-y-2">
                          <div className="flex items-center justify-between text-sm">
                            <span className="text-muted-foreground">{t.rewards.nextReward}</span>
                            <span className="font-medium">{nextReward.remaining} {t.rewards.pointsToNext}</span>
                          </div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {nextReward.icon} {locale === "ar" ? nextReward.nameAr : nextReward.nameEn} — {nextReward.points} {t.rewards.pointsNeeded}
                          </p>
                        </div>
                      )}

                      <p className="text-xs text-center text-muted-foreground">{t.rewards.earnMore}</p>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="shadow-sm border-border/50">
                    <CardHeader className="pb-3">
                      <CardTitle className="text-base">{t.rewards.rewards}</CardTitle>
                    </CardHeader>
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <motion.span
                                animate={canRedeem ? { scale: [1, 1.15, 1] } : { scale: 1 }}
                                transition={{ repeat: canRedeem ? Infinity : 0, duration: 2, ease: "easeInOut" }}
                                className="text-2xl"
                              >
                                {tier.icon}
                              </motion.span>
                              <div>
                                <p className="font-medium text-sm">{locale === "ar" ? tier.nameAr : tier.nameEn}</p>
                                <div className="flex items-center gap-1.5">
                                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full tier-badge-${tier.tier}`}>
                                    {tier.tier.charAt(0).toUpperCase() + tier.tier.slice(1)}
                                  </span>
                                  <p className="text-xs text-muted-foreground">{tier.points} {t.rewards.pointsNeeded}</p>
                                </div>
                              </div>
                            </div>
            ...
            <TabsContent value="giftcards" className="space-y-6 mt-4">
              ...
              <Card className="shadow-sm border-border/50">
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {CARD_TEMPLATES.map((tmpl) => (
                        ...
                        >
                          ...
                          <p className="text-[10px] mt-1 font-medium">{t.rewards.template[tmpl.key]}</p>
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  {gcRecipient && effectiveAmount > 0 && (
                    ...
                    >
                      <div className={`bg-gradient-to-br ${getTemplateGradient(gcTemplate)} p-5 text-white relative`}>
                        {/* Decorative pattern */}
                        ...
                        <div className="relative z-10">
                          <div className="flex items-center justify-between mb-6">
                            <span className="text-2xl">{getTemplateIcon(gcTemplate)}</span>
                  ...
                  <Button
                    ...
                    disabled={isPurchasing || !gcRecipient || !gcFrom || effectiveAmount <= 0}
                  >
                    {isPurchasing ? (
              ...
              <Card className="shadow-sm border-border/50">
                <CardHeader className="pb-3">
                  <CardTitle className="text-base">{t.rewards.myGiftCards}</CardTitle>
                ...
                <CardContent className="space-y-4">
                  <div className="flex gap-2">
                    ...
                    >
                      {isLoadingGc ? <Loader2 className="size-4 animate-spin" /> : <Search className="size-4" />}
                    </Button>
                  </div>
                  ...
                  <AnimatePresence mode="wait">
                    {giftCards.length === 0 && gcLookupName && !isLoadingGc ? (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center py-6">
                        ...
                        <p className="text-sm text-muted-foreground">{t.rewards.noGiftCards}</p>
                      </motion.div>
                    ) : (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3 max-h-64 overflow-y-auto">
                        {giftCards.map((gc) => (
                          <div key={gc.id} className="p-3 rounded-lg border bg-card space-y-2">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                <span className="text-lg">{getTemplateIcon(gc.template)}</span>
                                <div>
                              ...
                              <Badge variant={gc.isRedeemed ? "outline" : "default"} className={gc.isRedeemed ? "" : "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300 border-0"}>
                                {gc.isRedeemed ? t.rewards.redeemed : t.rewards.active}
                              </Badge>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <code className="text-xs bg-muted px-2 py-0.5 rounded font-mono">{gc.code}</code>
                              <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => copyCode(gc.code)}>
