...
async function main() {
  ...
  for (let d = 29; d >= 0; d--) {
    ...
    for (let o = 0; o < ordersToday; o++) {
      ...
      const deliveryFee = type === OrderType.delivery ? 4.99 : 0;
      const tip =
        Math.random() > 0.5
          ? Math.round(subtotal * (0.15 + Math.random() * 0.1) * 100) / 100
          : 0;
      ...
      await db.order.create({
        data: {
          ...
          total,
          paymentMethod:
            Math.random() > 0.5 ? PaymentMethod.card : PaymentMethod.cash,
          paymentStatus: PaymentStatus.paid,
          serverName: ["Sarah", "Layla", "Yusuf"][
            Math.floor(Math.random() * 3)
          ],
          estimatedReady: new Date(date.getTime() + 25 * 60_000),
          completedAt:
            status === OrderStatus.completed
              ? new Date(date.getTime() + 30 * 60_000)
              : null,
          items: { create: items },
        },
      });
  ...
  console.log(
    `✅ Generated ${orderCounter - 2000} historical orders over 30 days`
  );
}
